define(["build/fileUtils", "bdParse", "./symbols"], function(fileUtils, bdParse, symbols) {
	...
	function paramModified(param){
		// param is a symbol
		if(param.setterLocs.length){
			return true;
		}
		for(var p in param.props){
			if(paramModified(param.props[p])){
				return true;
			}
		}
		return false;
	...
	var sause = {
		...
		afterReturn:function(){
			if(this.expr.type===bdParse.NameNode){
				var s = symbols.getSymbol(this.expr.token.value, currentFrame);
				if(s.doc){
					this.doc = s.doc;
				}
			}else{
		...
		afterLexicalVariable:function(sause){
			...
			if(this.doc){
				symbols.getSymbol(this.nameToken.value, currentFrame).doc = this.doc;
			}
		}
	};
