define(["build/buildControl", "bdParse", "./symbols"], function(bc, bdParse, symbols){
	...
	function processCommentBlock(block){
		var commonWhitespace, minLength= 1000;
		block = block.map(function(comment){
			// peel off the comment tokens
			var text;
			...
			if(/^\s*$/.test(text)){
				...
				}else if(match==commonWhitespace){
					// short curcuit
				}else{
	...
	var sause = {
		...
		beforeFunctionLiteral:function(){
			...
			if(this.leftBrace.comment){
				this.doc = parseDocComment(this.leftBrace.comment);
			}
		},
