define(["build/buildControl", "bdParse"], function(bc, bdParse) {
	...
	var
		...
		retokenizeDojoPCodeBlocks = function(
			tokens //(array of tokens) tokens scan and retokenize
		){
			///
			// Scans tokens for dojo doc "/*=====" blocks. Converts any blocks found into tokens
			// and replaces block comment tocken with converted tokens.
			var filtered= [],
			...
			while(i<end){
				...
				if(item.type==tBlockComment && /\/\*=====/.test(item.value)){
					...
					newTokens = bdParse.tokenize(bdParse.split(text)).map(function(token){ token.location.offsetLine(offset); return token; });
	...
	return function(resource, callback) {
		try{
			var text = bdParse.split(resource.getText()),
				tokens = bdParse.tokenize(text);
			if(1 || bc.bdDoc.dojoConsumePCodeBlocks){
				tokens = retokenizeDojoPCodeBlocks(tokens);
			}
			tokens = filterComments(tokens);
			resource.text = text;
			resource.tokens = tokens;
			resource.ast = bdParse.parse(tokens);
		}catch(e){
